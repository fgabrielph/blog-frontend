<script setup lang="ts">
    ...
    const postId = computed(() => String(route.query.id ?? ''))

    const { state, categories, categoriesPending, isSubmitting, postPending, postError, activeTab, tabItems, validate, onSubmit } = await useUpdatePost(postId)
        ...
        <!-- 12-Column Grid Layout -->
            ...
            <main class="lg:col-span-9">
                <UCard class="shadow-sm">
                    <div v-if="postPending" class="py-16 text-center text-gray-400">
                        Loading post...
                    </div>

                    <div v-else-if="postError" class="py-16 text-center text-red-500">
                        Failed to load post.
                    </div>

                    <UForm v-else :validate="validate" :state="state" class="space-y-6" @submit="onSubmit">
                        ...
                        <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-100 dark:border-gray-800">
                            ...
                            <UButton
                                ...
                                :disabled="isSubmitting"
                            >
                                {{ isSubmitting ? 'Saving...' : (state.is_published ? 'Update Post' : 'Save Draft') }}
                            </UButton>
                        </div>
                    </UForm>
                </UCard>
